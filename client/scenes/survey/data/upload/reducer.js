...
export function getImportedMarkdown(state) {
  return state.scenes.survey.data.upload.markdown;
}

export function getResolvedIssueId(state) {
  return state.scenes.survey.data.upload.resolvedIssueIds;
}

export function getCollaboratorEmails(state) {
  return state.scenes.survey.data.upload.collaboratorEmails;
}

export function getCollaboratorOptions(state) {
  return state.scenes.survey.data.upload.collaboratorOptions;
}

export function getNewIssues(state) {
  return state.scenes.survey.data.upload.newIssues;
}

export function getCommentPeriodUnit(state) {
  return state.scenes.survey.data.upload.commentPeriodUnit;
}

export function getCommentPeriodValue(state) {
  return state.scenes.survey.data.upload.commentPeriodValue;
}
export function getProjectScorecardStatus(state) {
  const scorecard = state.scenes.survey.data.upload.scorecard;
  return (
    !isEmpty(scorecard) &&
...
export function getProjectScorecard(state) {
  return state.scenes.survey.data.upload.scorecard;
}
